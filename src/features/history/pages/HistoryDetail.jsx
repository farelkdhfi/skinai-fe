...
export default function HistoryDetail() {
    ...
    // PERUBAHAN 2: useState isDeleting/isDeleted dihapus, gantikan dengan
    // status bawaan mutation (isPending) dari useDeleteHistory().
    ...
    // PERUBAHAN 3: handleDelete sekarang manggil deleteMutation.mutate().
    // onSuccess di sini menangani navigate + tutup modal (pengganti
    // setIsDeleted(true) manual). onSuccess di dalam useDeleteHistory sendiri
    // tetap jalan duluan untuk invalidate ['history'] (list dashboard &
    // detail lain otomatis refresh).
    const handleDelete = () => {
        deleteMutation.mutate(historyId, {
            ...
            onError: (err) => {
                if (err.response?.status === 404) {
                    // Data sudah tidak ada, anggap berhasil (perilaku sama seperti sebelumnya)
